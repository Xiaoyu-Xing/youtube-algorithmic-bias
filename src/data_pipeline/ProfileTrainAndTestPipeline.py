...
def train_and_test_profile(subreddit: str,
                           ...
                           cookie_path: str,
                           video_json_path: str,
                           sequence_number) -> Tuple[str, str]:
    videos = []
    if video_json_path != "":
        with open(video_json_path) as f:
            videos: List[str] = json.load(f)
    batch_size: int = len(videos) if "stateless" in tag and len(videos) > 0 \
        else settings.training_batch_size
    ...
    with FireFoxBrowser(cookie_path) as browser:
        tester = YouTubeQueryTester(browser, tag, subreddit, settings.keyword, sequence_number)
