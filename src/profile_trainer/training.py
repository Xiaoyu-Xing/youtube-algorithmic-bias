...
class Trainer:
    ...
    def get_all_list(self):
        for (cur_dir, sub_dir, cur_files) in os.walk(self.train_directory):
            ...
            if 'extended' in cur_dir and ('diversity' in cur_dir or 'RNG' in cur_dir):
                for file in cur_files:
                    if 'json' in file:
                        v_list, _, _ = self.read_json(os.path.join(cur_dir, file))
    ...
    def _setup_browser(self, cookies_path):
        if settings.ads_block:
            fp = FirefoxProfile(settings.firefox_profile_rich_config)
        else:
        ...
        browser.get(settings.initial_website)
        time.sleep(3)
        try:
    ...
    def train_a_list(self, train_list,
                     ...
                     training_cookie=settings.training_cookie_saving_path):
...
@click.option("--tc", "training_cookie", default=settings.training_cookies_list[0],
              help="Training cookie to save after training")
